...
export default function ZhPracticePage() {
  ...
  } = useRecordingPipeline({ transcribeUrl: "/api/zh/transcribe" });
  ...
  const CACHE_KEY = `zh_poster_img_${exerciseId}`;

  useEffect(() => {
    fetch(`/api/zh/exercises/${exerciseId}`)
      .then((r) => r.json())
  ...
  const handleBack = () => {
    ...
    if (hasAnything) {
      if (confirm("You have recordings. Leave without submitting?")) router.push("/zh");
    } else {
      router.push("/zh");
    }
  };
  ...
  const handleSaveAttempt = async () => {
    ...
    try {
      ...
      const evalRes = await fetch("/api/zh/evaluate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmAttempt = async (attemptIdx: number) => {
    ...
    for (const a of toDelete) {
      await fetch(`/api/zh/practice?id=${a.id}`, { method: "DELETE" }).catch(() => {});
    }
    router.push(`/zh/results/${chosen.id}`);
  };
        ...
        <main>
          <div className="container" style={{ paddingTop: 20 }}>
            <div className="empty-state">
              ...
              <button className="btn btn-primary" style={{ marginTop: 16, maxWidth: 200 }} onClick={() => router.push("/zh")}>Back to Home</button>
            </div>
          </div>
        </main>
      </>
  ...
  const exerciseTypeLabel = isReading ? "朗读 (Reading Aloud)" : "看图说话 (Picture Conversation)";
  ...
  const submitLabel = submitting
    ...
    : busyAfterRecording
      ...
      : attempts.length === 0
        ? "提交评分 (Submit) — Attempt 1"
        : `提交评分 (Submit) — Attempt ${attempts.length + 1} (${3 - attempts.length} left)`;
    ...
    <>
      <header className="page-header">
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            ...
            <div className="subtitle">{exerciseTypeLabel} &middot; {exercise.difficulty}</div>
          </div>
        </div>
      </header>
      ...
      <main>
        <div className="container" style={{ paddingTop: 12 }}>
          ...
          {isReading ? (
            <>
              <div className="section-label">朗读短文</div>
