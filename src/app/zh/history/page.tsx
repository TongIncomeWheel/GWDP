...
export default function ZhHistoryPage() {
  ...
  const handleDelete = async (id: number, e: React.MouseEvent) => {
    ...
    await fetch(`/api/zh/practice?id=${id}`, { method: "DELETE" });
    ...
    <>
      <header className="page-header">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1>练习历史</h1>
            <div className="subtitle">Mandarin Practice History</div>
          </div>
          <button
        ...
        <div className="container" style={{ paddingTop: 16 }}>
          {!loading && history.length > 0 && (
            <>
              ...
              >
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>练习连续天数</div>
          ...
          {loading ? (
            ...
            <div className="empty-state">
              ...
              <p>暂无练习记录。</p>
              <p style={{ marginTop: 4, fontSize: 13 }}>完成一次练习后，记录将显示在此处。</p>
            </div>
          ) : (
            history.map((h) => {
              ...
              return (
                <div
                  ...
                  onClick={() => router.push(`/zh/results/${h.id}`)}
                >
                  {h.isClosed && (
                    <span
                      style={{
                  ...
                  <div className="history-item">
                    ...
                    <div className="hi-info">
                      ...
                      <div className="hi-meta">
                        {dateStr} · {exerciseTypeLabel(h.exerciseType)} · AI: {h.totalScore}/{h.maxScore}
                        {hasParentScore && (
                          <> · 家长: {h.parentTotalScore}/{h.maxScore}</>
                        )}
                      </div>
                    </div>
                    <button
