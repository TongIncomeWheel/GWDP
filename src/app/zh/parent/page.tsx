...
export default function ZhParentDashboard() {
  ...
  const exerciseTypeLabel = (type: string) => {
    if (type === "READING") return "朗读";
    if (type === "STIMULUS") return "看图说话";
    return type;
  };

  const exerciseTypeShort = (type: string) => {
    if (type === "READING") return "朗读";
    if (type === "STIMULUS") return "图";
    return type;
  };
    ...
    <>
      <header className="page-header">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1>家长面板</h1>
            <div className="subtitle" style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, marginTop: 2 }}>
              Mandarin Parent Dashboard
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => router.push("/zh/parent/settings")}
        ...
        <div className="container" style={{ paddingTop: 16 }}>
          {loading ? (
            ...
            <>
              ...
              {trendSessions.length >= 2 && (
                <div className="card">
                      ...
                      return (
                        <div key={h.id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                          ...
                          <div style={{ fontSize: 9, color: "var(--text-muted)", textAlign: "center", lineHeight: 1.2 }}>
                            {exerciseTypeShort(h.exerciseType)}
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div style={{ display: "flex", gap: 12, marginTop: 10, fontSize: 11, color: "var(--text-muted)" }}>
                    <span>← 较早</span>
                    <span style={{ marginLeft: "auto" }}>最近 →</span>
                  </div>
                </div>
              )}
              ...
              {hasInsights && (
                <div className="card">
                  <div className="card-title">学习分析</div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>
                    基于最近 {insightBase.length} 次已评估练习
                  </div>
              ...
              {history.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p>暂无练习记录。</p>
                  <p style={{ marginTop: 4, fontSize: 13 }}>孩子完成练习后，记录将显示在此处。</p>
                </div>
              ) : (
                history
                  ...
                  .map((h) => {
                    ...
                    const dateStr = date.toLocaleDateString("zh-SG", { day: "numeric", month: "short", year: "numeric" });
                    ...
                    return (
                      <div
                        ...
                        onClick={() => router.push(`/zh/parent/session/${h.id}`)}
                      >
                        <div className="history-item">
                          ...
                          <div className="hi-info">
                            ...
                            <div className="hi-meta">
                              {dateStr} &middot; {exerciseTypeLabel(h.exerciseType)} &middot;{" "}
                              {h.isEvaluated ? `${h.totalScore}/${h.maxScore}` : h.errorMessage ? "错误" : "待评分"}
                              {h.parentTotalScore != null && <span> &middot; 家长: {h.parentTotalScore}</span>}
                            </div>
                          </div>
