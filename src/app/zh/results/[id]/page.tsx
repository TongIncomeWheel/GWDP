...
export default function ZhResultsPage() {
  ...
  const fetchHistory = useCallback(() => {
    fetch(`/api/zh/practice?id=${historyId}`)
      .then((r) => r.json())
      .then((data) => {
        setHistory(data);
        setLoading(false);
  ...
  const handleClose = async () => {
    ...
    setClosing(true);
    try {
      const res = await fetch("/api/zh/practice/close", {
        method: "POST",
      ...
      <main>
        <div className="container" style={{ paddingTop: 16 }}>
          ...
          >
            ...
            <button
              className="btn btn-outline"
              onClick={() => router.push(`/zh/practice/${history.exerciseId}`)}
            >
              Try Again
            </button>
            <button
            ...
            <button
              className="btn btn-outline"
              onClick={() => router.push(`/zh/parent/session/${history.id}`)}
              style={{
