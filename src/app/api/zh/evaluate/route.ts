...
import { getZhPracticeHistoryById, getZhExerciseById, updateZhEvaluationResult, setZhEvaluating, getEffectiveApiKey } from "@/lib/zh-db";
import { audioToBase64 } from "@/lib/audio-service";
import { evaluateZhWithGemini } from "@/lib/zh-gemini";
...
export async function POST(request: NextRequest) {
  ...
  const history = await getZhPracticeHistoryById(historyId);
  if (!history) {
    return NextResponse.json({ error: "Practice history not found" }, { status: 404 });
  }

  await setZhEvaluating(historyId, true, null);

  const exercise = await getZhExerciseById(history.exerciseId);
  if (!exercise) {
    await setZhEvaluating(historyId, false, "Corresponding exercise not found.");
  ...
  if (!apiKey) {
    ...
    await setZhEvaluating(historyId, false, errorMessage);
  ...
  try {
    ...
    const result = await evaluateZhWithGemini(history, exercise, apiKey, audioBlobs);
    ...
    await updateZhEvaluationResult(historyId, evalFields);
    ...
    await updateZhEvaluationResult(historyId, errorFields);
