...
import {
  getZhExerciseById,
  claimZhImageGeneration,
  finishZhImageGeneration,
  clearZhImageGenerating,
  prepopulateZhExercisesIfNeeded,
  getEffectiveApiKey,
} from "@/lib/zh-db";
...
function extractImage(node: unknown, depth = 0): { b64: string; mime: string } | null {
  ...
  for (const key of [
    "output_image", "output", "content", "parts", "steps",
  ...
  for (const [k, v] of Object.entries(obj)) {
    if (
      [
        "id", "model", "object", "status", "usage", "created", "updated",
        "service_tier", "type", "role", "text", "finish_reason",
        "index", "prompt_tokens", "completion_tokens", "total_tokens",
      ].includes(k)
    )
      continue;
...
async function tryInteractionsAPI(
  ...
  for (const model of models) {
    try {
      ...
      if (img) {
        console.log(`[ZH POSTER] ${model}: success via Interactions API`);
      ...
      const step0 = steps?.[0];
      const step0Keys =
        step0 && typeof step0 === "object" ? Object.keys(step0 as object).join(",") : "none";
      const snippet = JSON.stringify(step0 || data).slice(0, 600);
      console.log(
        `[ZH POSTER] ${model}: no image found. top-keys=${Object.keys(data).join(",")} step[0]-keys=${step0Keys} snippet=${snippet}`
      );
      errors.push(
        `${model}: no image in Interactions response (top-keys: ${Object.keys(data).join(",")}, step[0]-keys: ${step0Keys})`
      );
    } catch (e) {
...
async function tryGenerateContent(
  ...
  for (const model of models) {
    try {
      ...
      const img = extractImage(data as Record<string, unknown>);
      if (img) {
        console.log(`[ZH POSTER] ${model}: success via generateContent`);
        return `data:${img.mime};base64,${img.b64}`;
      }
...
export async function GET(
  ...
  try {
    ...
    const exercise = await getZhExerciseById(Number(id));
...
export async function POST(
  ...
  try {
    ...
    const { description, force } = await request.json();
    const exerciseId = Number(paramId);
    ...
    if (!force) {
      ...
      const claim = await claimZhImageGeneration(exerciseId);
    ...
    if (!apiKey) {
      await clearZhImageGenerating(exerciseId).catch(() => {});
    ...
    if (imageUrl) {
      ...
      await finishZhImageGeneration(exerciseId, imageUrl).catch(() => {});
