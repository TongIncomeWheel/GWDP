...
import {
  getAllZhPracticeHistoryMeta,
  getZhPracticeHistoryById,
  insertZhPracticeHistory,
  updateZhParentGrading,
  updateZhPracticeHistory,
  deleteZhPracticeHistoryById,
} from "@/lib/zh-db";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const history = await getZhPracticeHistoryById(Number(id));
      if (!history) return NextResponse.json({ error: "Not found" }, { status: 404 });
      return NextResponse.json(history);
    }
    return NextResponse.json(await getAllZhPracticeHistoryMeta());
  } catch (e) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const id = await insertZhPracticeHistory({
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existing = await getZhPracticeHistoryById(Number(body.id));
    if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
    const updated: PracticeHistory = { ...existing, ...body };
    await updateZhPracticeHistory(updated);
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 500 });
  }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await deleteZhPracticeHistoryById(Number(id));
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ error: String(e) }, { status: 500 });
  }
